...
2. 变量用static告知编译器，自己仅在变量的作用范围内可见。这一点是它与全局变量的区别。 作用范围有，文件内，类内，函数内。
3. 在可重入的函数中，我们一定要避免函数中使用static变量
4. 函数中必须要使用static变量的情况:比如当某函数的返回值为指针类型时，则必须是static的局部变量的地址作为返回值，若为auto类型，则返回为错指针。 

|                       | 全局变量             | 静态变量                     | 静态局部       | 局部变量       |
| --------------------- | -------------------- | ---------------------------- | -------------- | -------------- |
| 存储器                | 静态存储区域         | 静态存储区域                 | 静态存储区域   | 内存的栈区     |
| 作用域分              | 整个工程文件         | 从文件中定义它的位置开始有效 | 定义它的函数内 | 定义它的函数内 |
| static修饰<br/>作用域 | 只在定义的文件中有效 |                              |                |                |
